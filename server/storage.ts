...
export class DatabaseStorage implements IStorage {
  ...
  async seedDemoUserData(userId: string): Promise<void> {
    const [existingPositions, existingEvents] = await Promise.all([
      this.getPositions(userId),
      this.getEngineEvents(userId, 1),
    ]);
    if (existingPositions.length > 0 && existingEvents.events.length > 0) {
      return;
    }
    ...
    const investmentAmounts = [
      5000_000000, // 5,000 USDT
      3000_000000, // 3,000 USDT
      2000_000000, // 2,000 USDT
    ];

    if (existingPositions.length === 0) {
      // Positions with ~60 days of accumulated profit
      for (let i = 0; i < selectedStrategies.length; i++) {
        ...
        const accumulatedProfit = Math.floor((principal * monthlyReturnBps * 60) / (30 * 10000));
        const currentValue = principal + accumulatedProfit;

        await db.insert(positions).values({
          userId,
          strategyId: strategy.id,
          principal: Math.floor(principal / 1000000).toString(),
          currentValue: Math.floor(currentValue / 1000000).toString(),
      ...
      const startingValue = 8000_000000;
      const portfolioData: InsertPortfolioSeries[] = [];
      for (let i = 90; i >= 0; i--) {
        const daysSinceStart = 90 - i;
        const growthFactor =
          1 +
          daysSinceStart * 0.003 +
          Math.sin(daysSinceStart * 0.5) * 0.02 +
          (Math.random() - 0.5) * 0.01;
        const additionalDeposits =
          (daysSinceStart >= 30 ? 1000_000000 : 0) +
          (daysSinceStart >= 60 ? 1000_000000 : 0);
        portfolioData.push({
          userId,
          date: daysAgo(i).toISOString().split("T")[0],
          value: Math.floor((startingValue + additionalDeposits) * growthFactor).toString(),
        });
      }
      ...
      await db.insert(operations).values({
        userId, type: "DEPOSIT_USDT", status: "completed", asset: "USDT",
        amount: "8000000000", fee: "0",
      }).onConflictDoNothing();
      await db.execute(sql`UPDATE operations SET created_at = ${daysAgo(90)} WHERE user_id = ${userId} AND type = 'DEPOSIT_USDT' AND amount = '8000000000'`);

      for (let i = 0; i < selectedStrategies.length; i++) {
        await db.insert(operations).values({
          userId, type: "INVEST", status: "completed", asset: "USDT",
          amount: investmentAmounts[i].toString(),
          strategyId: selectedStrategies[i].id,
          strategyName: selectedStrategies[i].name,
        });
      }
      const day89 = daysAgo(89);
      await db.execute(sql`UPDATE operations SET created_at = ${day89} WHERE user_id = ${userId} AND type = 'INVEST' AND created_at > ${day89}`);

      for (const [daysBack, amount] of [[60, "1000000000"], [30, "1000000000"]] as const) {
        await db.insert(operations).values({
          userId, type: "DEPOSIT_USDT", status: "completed", asset: "USDT",
          amount, fee: "0",
        });
        const cutoff = daysAgo(daysBack);
        await db.execute(sql`UPDATE operations SET created_at = ${cutoff} WHERE user_id = ${userId} AND type = 'DEPOSIT_USDT' AND amount = ${amount} AND created_at > ${cutoff}`);
      }

      for (let week = 1; week <= 8; week++) {
        const accrualDate = daysAgo(90 - week * 7);
        for (let i = 0; i < selectedStrategies.length; i++) {
          const op = await db.insert(operations).values({
            userId, type: "PROFIT_ACCRUAL", status: "completed", asset: "USDT",
            amount: Math.floor(investmentAmounts[i] * 0.007).toString(),
            strategyId: selectedStrategies[i].id,
            strategyName: selectedStrategies[i].name,
          }).returning();
          if (op[0]) {
      ...
      const withdrawOp = await db.insert(operations).values({
        userId, type: "WITHDRAW_USDT", status: "completed", asset: "USDT",
        amount: "500000000", fee: "1000000",
        txHash: "0xdemo" + Math.random().toString(36).substring(2, 15),
      }).returning();
      if (withdrawOp[0]) {
        await db.execute(sql`UPDATE operations SET created_at = ${daysAgo(7)} WHERE id = ${withdrawOp[0].id}`);
      }

      await this.updateVault(userId, "principal", "10000000000");
      await this.updateVault(userId, "profit", "850000000");
      await this.updateVault(userId, "taxes", "150000000");
    }

    // Batch-insert all engine events (one round trip)
    const tierNames = ["Активный", "Сбалансированный", "Стабильный"];
    const dailyAmounts = [18.5, 12.3, 7.8];
    const eventRows: (typeof engineEvents.$inferInsert)[] = [];

    for (let d = 29; d >= 0; d--) {
      for (let i = 0; i < selectedStrategies.length; i++) {
        const variation = 1 + Math.sin(d * 0.7 + i) * 0.15;
        eventRows.push({
          userId,
          strategyId: selectedStrategies[i].id,
          type: "daily_pnl",
          severity: "info",
          message: `Дневной доход по стратегии ${selectedStrategies[i].name}`,
          payloadJson: { amount: Math.round(dailyAmounts[i] * variation * 100) / 100, tier: tierNames[i] },
          createdAt: daysAgo(d),
        });
      }
    }

    const settlementAmounts = [112.30, 76.10, 98.40];
    for (let m = 0; m < 3; m++) {
      eventRows.push({
        userId, type: "settlement", severity: "info",
        message: "Ежемесячный расчёт портфеля",
        payloadJson: { amount: settlementAmounts[m] },
        createdAt: daysAgo((m + 1) * 30),
      });
    }

    eventRows.push({
      userId, type: "deposit", severity: "info",
      message: "Пополнение зачислено",
      payloadJson: { amount: 8000 },
      createdAt: daysAgo(90),
    });

    eventRows.push({
      userId, type: "withdrawal", severity: "info",
      message: "Вывод выполнен",
      payloadJson: { amount: 500 },
      createdAt: daysAgo(7),
    });

    await db.insert(engineEvents).values(eventRows);
  }
