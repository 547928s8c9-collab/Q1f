...
class MarketSimService extends EventEmitter {
  ...
  private async tick(): Promise<void> {
    ...
    for (const symbol of this.symbols) {
      try {
        ...
        const price = this.computePrice(candle, simNow, symbol);
  ...
  private async getCandleForSymbol(symbol: string, simNow: number): Promise<Candle | null> {
    ...
    const result = await loadCandles({
      exchange: DEFAULT_MARKET_EXCHANGE,
      symbol,
      timeframe: "1m",
      startMs: candleStart,
  ...
  private computePrice(candle: Candle, simNow: number, symbol: string): number {
    return computeDeterministicPrice(candle, simNow, symbol, DEFAULT_NOISE_SEED);
  }
}
