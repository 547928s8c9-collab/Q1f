...
(async () => {
  ...
  // Auto-bootstrap market data (non-blocking, runs in background)
  if (process.env.AUTO_MARKET_BOOTSTRAP !== "false") {
    setTimeout(async () => {
      try {
        logger.info("Market bootstrap started", "market-bootstrap");
        const { getMarketUniverse } = await import("./app/marketUniverse");
        const { bootstrapUniverse } = await import("./market/binanceVisionImporter");
        const { calibrateFromDb } = await import("./app/marketCalibrationService");
        
        const universe = await getMarketUniverse(storage);
        const monthsBack = parseInt(process.env.MARKET_BOOTSTRAP_MONTHS_BACK || "6", 10);
        const daysBack = parseInt(process.env.MARKET_BOOTSTRAP_DAYS_BACK || "14", 10);
        
        const bootstrapResult = await bootstrapUniverse(
          universe,
          { monthsBack, daysBackForCurrentMonth: daysBack },
          storage
        );
        
        logger.info("Market bootstrap completed", "market-bootstrap", {
          markets: bootstrapResult.markets.length,
          totalInserted: bootstrapResult.totalInserted,
          totalSkipped: bootstrapResult.totalSkipped,
        });
        
        // Calibrate markets
        const windowDays = parseInt(process.env.MARKET_CALIBRATION_WINDOW_DAYS || "180", 10);
        for (const market of universe) {
          try {
            await calibrateFromDb({
            ...
            logger.warn("Calibration failed for market", "market-bootstrap", {
              symbol: market.symbol,
              timeframe: market.timeframe,
              error: err,
            });
        ...
        logger.info("Market calibration completed", "market-bootstrap");
      } catch (err) {
        logger.error("Market bootstrap failed", "market-bootstrap", {}, err);
      }
    }, 1000); // Start 1s after server start
  }
})();
